...
void onewiredevice::sendCommand()
{
    if (!plugin_interface) return;
        plugin_interface->setStatus(romid + "=" + command.text());
}
...
void onewiredevice::changename()
{
	bool ok;
retry:
	QString Name = inputDialog::getTextPalette(this, cstr::toStr(cstr::Name), cstr::toStr(cstr::Name2dot), QLineEdit::Normal, name, &ok, Qt::Dialog, parent);
...
void onewiredevice::setListTreeItem(QTreeWidgetItem *item)
{
    listTreeItem = item;
}

void onewiredevice::clearListTreeItem()
{
...
void onewiredevice::assignMainValue(double value)
{
    if (logisdom::isNA(value)) {
        ...
        return; }
    if (plugin_interface)plugin_interface->setStatus(romid + QString("=%1").arg(value, 0, 'f', 3));
    else assignMainValueLocal(value);
}
...
void onewiredevice::setPluginMainValue(double value, bool enregistremode)
{
    setMainValue(AXplusB.result(value), enregistremode);
}
...
{
    if (!isVirtualFamily()) if ((!master) && (!plugin_interface)) return;
    str +="\n" One_Wire_Device "\n";
    if (All)
    {
