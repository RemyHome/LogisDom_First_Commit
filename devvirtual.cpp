...
void devvirtual::saveDeviceConfig()
{
    ...
    for (int n=0; n<parent->configwin->devicePtArray.count(); n++)
    {
        ...
        if (index != -1)
        {
            QString nom = inputDialog::getTextPalette(this, cstr::toStr(cstr::Name), tr("Confirm device name : "), QLineEdit::Normal, devName, &ok, Qt::Dialog, parent);
            if (!ok) return;
