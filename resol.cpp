...
{
	QString hex;
	for (int n=0; n<data.length(); n++) hex += QString("%1").arg((unsigned char)data[n], 2, 16, QChar('0'));
	return hex.toUpper();
}
...
double resol::getParameter(QByteArray &frameStr, int &frame, int &pos, int &size)
{
	...
	QString logtxt;
	for (int n=0; n<frameN.length(); n++) logtxt += QString("[%1]").arg((unsigned char)frameN[n]);
	...
	if (size == 1) total = getByte(frameN, pos);
	if (size == 2) total = getByte(frameN, pos) + 256 * getByte(frameN, pos + 1);
	if (size == 3) total = getByte(frameN, pos) + 256 * getByte(frameN, pos + 1) + 256 * 256 * getByte(frameN, pos + 2);
	if (size == 4) total = getByte(frameN, pos) + 256 * getByte(frameN, pos + 1) + 256 * 256 * getByte(frameN, pos + 2) + 256 * 256 * 256 * getByte(frameN, pos + 3);
	int T = (int) total;
	bool s = false;
	if (T & 0x8000)
	{
